  ...
  return (
    <div className="slide-in">
      ...
      <div className="grid gap-6 mb-6" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))' }}>
        <StatCard title="Today's Calls" value="47" icon={<PhoneCall />} trend="up" trendValue="12%" />
        <StatCard title="Hot Leads" value={recentLeads.filter(l => l.category === 'hot' || l.category === 'High').length} icon={<Flame color="#ef4444" />} trend="up" trendValue="5%" />
        <StatCard title="Pipeline Value" value="₹4.2L" icon={<TrendingUp />} trend="up" trendValue="18%" />
        <StatCard title="Pending Follow-ups" value="5" icon={<Clock color="#f59e0b" />} trend="down" trendValue="2%" />
      </div>
      ...
      <div className="grid gap-6" style={{ gridTemplateColumns: '2fr 1fr' }}>
        ...
        <div className="glass-card">
          <div className="flex justify-between items-center mb-4">
            ...
            <button className="btn btn-ghost text-teal" style={{ fontSize: '0.875rem' }}>View All</button>
          </div>
          <div className="flex" style={{ flexDirection: 'column', gap: '1rem' }}>
            {recentLeads.filter(l => l.category === 'hot' || l.category === 'High').map(lead => (
              <div key={lead.id} className="flex justify-between items-center" style={{ padding: '0.75rem', background: 'rgba(255,255,255,0.02)', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}>
                <div>
